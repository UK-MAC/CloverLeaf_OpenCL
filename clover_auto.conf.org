...
compile = make -B COMPILER=GNU MPI_COMPILER=ftn C_MPI_COMPILER=cc  CXX_MPI_COMPILER=CC OCL_VENDOR=CRAY OCL_LOCAL_WG_SIZE_XDIM=%WG_X% OCL_LOCAL_WG_SIZE_YDIM=%WG_Y% TEST_ID=%%ID%%
...
test = bash -c "if [[ `expr %WG_X% \* %WG_Y%` -eq 256 ]]; then aprun -q -n 1 -N 1 ./clover_leaf; else echo "20000"; fi"
...
clean = make clean; rm clover.in.tmp
