  ...
  DO c = 1, number_of_chunks
    IF(chunks(c)%task.EQ.parallel%task) THEN
      ...
      IF(use_OpenCL_kernels) THEN
        ! Bring data back from card ready for outputting
        CALL ocl_read_vis_buffers(chunks(c)%field%x_max   &
...
SUBROUTINE visit_lite
  ...
  name = 'clover_visit_lite'

  IF(first_call) THEN

    nblocks=number_of_chunks
    filename = "clover-lite.visit"
    u=get_unit(dummy)
    OPEN(UNIT=u,FILE=filename,STATUS='UNKNOWN',IOSTAT=err)
    WRITE(u,'(a,i5)')'!NBLOCKS ',nblocks
    CLOSE(u)
  ...
  IF ( parallel%boss ) THEN
    ...
    DO c = 1, number_of_chunks
      ...
      WRITE(step_name, '(i6)') step+100000
      step_name(1:1) = "."
      filename = trim(trim(name) //trim(chunk_name)//trim(step_name))//".lite.vtk"
      WRITE(u,'(a)')TRIM(filename)
    ENDDO
    CLOSE(u)
